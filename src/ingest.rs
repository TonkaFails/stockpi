...
pub async fn ingest_alpaca_stream(key: String, secret: String, tx: broadcast::Sender<String>) {
    ...
    loop {
        ...
        match connect_async(url.to_string()).await {
            Ok((ws_stream, _)) => {
                ...
                if let Err(e) = write
                    ...
                    .await
                {
                    tracing::error!("Failed during authentication: {}", e);
                ...
                if let Err(e) = write
                    ...
                    .await
                {
                    tracing::error!("Failed during subscription: {}", e);
                    sleep(Duration::from_secs(5)).await;
                    continue;
                }

                tracing::info!("Subscribed to: {:?}", SYMBOLS);
        ...
        tracing::warn!("Connection lost. Retrying in 5 seconds...");
        sleep(Duration::from_secs(5)).await;
    }
}