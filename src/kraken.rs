...
pub async fn ingest_kraken_stream(tx: broadcast::Sender<String>) {
    loop {
        tracing::info!("Connecting to Kraken XMR stream...");
        ...
        match connect_async(KRAKEN_URL).await {
            Ok((ws_stream, _)) => {
                ...
                tracing::info!("Subscribed to: {:?}", KRAKEN_SYMBOLS);
            ...
            Err(e) => tracing::error!("Kraken connection failed: {}", e),
        }

        tracing::warn!("Kraken connection lost. Retrying in 5 seconds...");
        sleep(Duration::from_secs(5)).await;
    }
}